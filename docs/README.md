...
- [adr/](adr/) — **[append-log]** architecture decision records (template: [adr/000-template.md](adr/000-template.md))
  ...
  - [adr/010-runtime-llm-conversation.md](adr/010-runtime-llm-conversation.md) — **proposed, feature parked**: the terms a spoken AI conversation partner must be built under (browser-side transcription, nothing persisted, a capability code on the route with rate limiting behind it, one tile that vanishes offline) — plus why a device-held API key was rejected
