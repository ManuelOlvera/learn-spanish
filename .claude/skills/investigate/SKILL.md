...
2. **Locate the layer.** Is the wrong behavior in `domain`/`application` (logic),
   ...
   hydration, offline, timing)? Most "UI bugs" in this codebase are really
   untested client-state or offline-queue bugs — confirm the layer, don't assume.
...
6. **Lock it with a regression test**, then hand off to
   `docs/workflows/fixing-a-bug.md` for review and verification.
